...
# pick_an_orange (reduces orange by 1 and puts statement "tasty"; if orange_count = 0 then puts dif statement "whomp")
	...
	def pick_an_orange
		if @orange_count >= 1
			@orange_count -= 1
			puts "You picked an orange. Mmmm tasty! Your tree has #{@orange_count} orange(s) left."
		else 
			puts "Sorry, there are no oranges to eat :("
		end
	end
