...
class OrangeTree
  ...
  def one_year_passes
    if @is_dead == false
      ...
      if @age <= 2
        ...
        @is_dead = true
        puts "The tree is dead.".colorize(:red) + ":("
        @orange_count = 0
        exit
      end
    return self
  ...
  def pick_an_orange
    ...
    if @orange_count > 0
      @orange_count -= 1
      puts "You picked an " + "orange".colorize(:yellow) + "! There are now " + "#{@orange_count} oranges".colorize(:yellow) + " left!"
    else
      puts "There are " + "no more oranges".colorize(:yellow) + " left!"
    end
  end

end # end OrangeTree class
